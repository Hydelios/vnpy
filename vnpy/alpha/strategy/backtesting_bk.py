...
import sys
# 获取当前文件所在目录
sys.path.append(r'D:\工作-gtzq\vnpy-master')  # 同时添加项目根目录
sys.path.append(r'D:\工作-gtzq\vnpy-master\vnpy\alpha')
    ...
    def show_performance(self, benchmark_symbol: str) -> pl.DataFrame:
        ...
        performance_df: pl.DataFrame = (
            self.daily_df.with_columns(
                # Gross strategy capital
                gross_balance=pl.col("total_pnl").cum_sum() + self.capital,
                # Daily cost
                cost=pl.col("commission") / pl.col("balance").shift(1),
            ).with_columns(
                # Cumulative cost
                cumulative_cost=pl.col("cost").cum_sum()
            ).with_columns(
                # Gross cumulative return
                cumulative_return=pl.col("gross_balance").pct_change().cum_sum(),
                # Net cumulative return
                net_cumulative_return=pl.col("balance").pct_change().cum_sum()
            ).with_columns(
                # Benchmark price
